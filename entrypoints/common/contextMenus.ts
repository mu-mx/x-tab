...
const getMenus = async (): Promise<Menus.CreateCreatePropertiesType[]> => {
  ...
  const contexts: Menus.ContextType[] = settings[SHOW_PAGE_CONTEXT_MENUS] ? ['all'] : ['action'];

  const _openAdminTab: Menus.CreateCreatePropertiesType = {
    id: ENUM_ACTION_NAME.OPEN_ADMIN_TAB,
    title:
      customMessages['common.openAdminTab'] + ` (${hotkeysMap?.[ENUM_ACTION_NAME.OPEN_ADMIN_TAB]})`,
    contexts,
  };
  ...
  const _sendAllTabs: Menus.CreateCreatePropertiesType = {
    ...
    title:
      customMessages['common.sendAllTabs'] + ` (${hotkeysMap?.[ENUM_ACTION_NAME.SEND_ALL_TABS]})`,
    contexts,
  ...
  const _sendLeftTabs: Menus.CreateCreatePropertiesType = {
    ...
    title:
      customMessages['common.sendLeftTabs'] + ` (${hotkeysMap?.[ENUM_ACTION_NAME.SEND_LEFT_TABS]})`,
    contexts,
